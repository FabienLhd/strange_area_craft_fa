function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    Menu.Index = 1
    Menu.Sprite = { Dictionary = "syncleak", Texture = "interaction_bgd", Color = { R = R, G = G, B = B, A = A } }
    Menu.Rectangle = nil
...
function RageUI.Menus:EditSpriteColor(R, G, B, A)
    if self.Sprite.Dictionary == "syncleak" then
...
function RageUI.Menus:SetSpriteBanner(TextureDictionary, Texture)
    self.Sprite = { Dictionary = TextureDictionary or "syncleak.ytd", Texture = Texture or "admin" }
    self.Rectangle = nil
end
